...
describe Comment do
	describe 'create a comment' do
		...
		it{Comment.find(:first, :conditions => {:user => @comment[:user], 
																						:comment => @comment[:comment], 
																						:article => @comment[:article]}).should eql(Comment.find(:first, :conditions => {:user => @comment[:user], 
																																																														:comment => @comment[:comment], 
	...
	describe 'delete a comment' do
		...
		it{Comment.find(:first, :conditions => {:user => @comment[:user], 
																						:comment => @comment[:comment], 
																						:article => @comment[:article]}).should eql(nil)}
	end
end